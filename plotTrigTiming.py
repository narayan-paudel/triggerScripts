...
SLCTankPulseTime = pd.read_hdf(str(args.input[0]),key="OfflineIceTopSLCTankPulsesHitTimeInterval")["value"].values
HLCTankPulseTime = pd.read_hdf(str(args.input[0]),key="OfflineIceTopHLCTankPulsesHitTimeInterval")["value"].values
events = pd.read_hdf(str(args.input[0]),key="OfflineIceTopHLCTankPulses")["Event"].values
print("events",set(events))
print("hlctank puslse time",HLCTankPulseTime[0])
print("hlctank pulse columns",pd.read_hdf(str(args.input[0]),key="OfflineIceTopSLCTankPulses").columns)
...
def plotScatterHLCHitInterval(zenChargeList,suffix):
	...
	ax = fig.add_subplot(gs[0])
	slcHitList = [zc.tankHitHLC for zc in zenChargeList]
	slcHitInterval = [zc.tankHitHLCTime for zc in zenChargeList]
	...
	plt.savefig(plotFolder+"/scatterHLCHitInterval"+str(suffix)+".png",transparent=False,bbox_inches='tight')
	plt.close()
...
def plotScatterSLCHitInterval(zenChargeList,suffix):
	...
	plt.savefig(plotFolder+"/scatterSLCHitInterval50muS"+str(suffix)+".png",transparent=False,bbox_inches='tight')
	plt.close()
