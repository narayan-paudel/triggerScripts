...
runList = [139834,139835,139836]
runListLabel = ["139834","139835","139836"]
liveTime = [7*60*60+59*60+58.365154,8*60*60+05.784291,8*60*60+00.183396]
# runList = np.linspace(138935,138952,18)
HG7rateList = []
HLCrateList = []
for run,idur in zip(runList,liveTime):  
  # with open("/home/enpaudel/icecube/triggerStudy/triggerRateListCombinedShort{}.txt".format(int(run)),"r") as f:
  with open("/home/enpaudel/icecube/triggerStudy/rateFilesEvents/triggerEventRate2024Test{}.txt".format(int(run)),"r") as f:
    iHG7rate = []
    iHLCrate = []
    for line in f:
      splits = line.split(" ")
...
dates = ["2024-09-04 19:58","2024-09-05 03:59","2024-09-05 11:59"]
fig = plt.figure(figsize=(8,5))
gs = gridspec.GridSpec(nrows=1,ncols=1)
ax = fig.add_subplot(gs[0])
ax.plot(runList,HG7rateList,"o",label=r"IT7HG",alpha=1)
ax.plot(runList,HLCrateList,"o",label=r"ITSMT",alpha=1)
ax.tick_params(axis='both',which='both', direction='in',labelsize=10)
secax = ax.secondary_xaxis('top')
secax.tick_params(axis='both',which='both', direction='in', labelsize=10)
# ax.tick_params(axis='x', labelrotation=90)
...
secax.set_xticklabels(dates)
ax.set_xticklabels(runListLabel)
# ax.set_xlabel(r"$Q_{tot}$ [m]", fontsize=22)
# ax.set_ylabel(r"$\theta$$^{\circ}$", fontsize=22)
ax.set_ylabel("rate [Hz]", fontsize=11)
# ax.set_xlabel("Q$_{tot} [VEM]$", fontsize=22)
...
plt.savefig(plotFolder+"/trigCount2024Test.pdf",transparent=False,bbox_inches='tight')
plt.savefig(plotFolder+"/trigCount2024Test.png",transparent=False,bbox_inches='tight')