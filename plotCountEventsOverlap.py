...
HG7NOHLCrateList = []
HLCrateList = []
missingRuns = [138629,138640,138659,138676,138679,138712,138744,138789,138838,138883]
for irun in runList:
  if irun in missingRuns:
    runList.remove(irun)
for run,idur in zip(runList,liveTime):
  if run not in missingRuns:
    ...
    with open("/home/enpaudel/icecube/triggerStudy/rateFilesEvents/triggerEventRate{}.txt".format(int(run)),"r") as f:
      iHG7rate = []
      iHG7WHLCrate = []
      iHG7NOHLCrate = []
      iHLCrate = []
      for line in f:
        ...
        if len(splits) > 2:
          ...
          iHG7WHLCrate.append(int(splits[5]))
          iHG7NOHLCrate.append(int(splits[4]))
    ...
    HG7WHLCrateList.append(sum(iHG7WHLCrate)/idur)
    HG7NOHLCrateList.append(sum(iHG7NOHLCrate)/idur)
...
ax = fig.add_subplot(gs[0])
ax.plot(runList,HG7rateList,".",label=r"IT7HG",alpha=1)
ax.plot(runList,HG7WHLCrateList,".",label=r"IT7HG w/ ITSMT",alpha=1)
ax.plot(runList,HG7NOHLCrateList,".",label=r"IT7HG w/o ITSMT",alpha=1)
ax.plot(runList,HLCrateList,".",label=r"ITSMT",alpha=1)
ax.tick_params(axis='both',which='both', direction='in', labelsize=10)
...
ax.set_ylim(0,30)
ax.legend()
plt.savefig(plotFolder+"/trigEventCount.pdf",transparent=False,bbox_inches='tight')
plt.savefig(plotFolder+"/trigEventCount.png",transparent=False,bbox_inches='tight')

# print("runList",runList[-tailLength:])

datesTail = [np.datetime64("2024-01-13"),np.datetime64("2024-01-13"),np.datetime64("2024-01-13"),np.datetime64("2024-01-14"),np.datetime64("2024-01-14")
,np.datetime64("2024-01-14"),np.datetime64("2024-01-15"),np.datetime64("2024-01-15"),np.datetime64("2024-01-15"),np.datetime64("2024-01-16")
,np.datetime64("2024-01-16"),np.datetime64("2024-01-16"),np.datetime64("2024-01-17"),np.datetime64("2024-01-17"),np.datetime64("2024-01-17")
,np.datetime64("2024-01-18"),np.datetime64("2024-01-18"),np.datetime64("2024-01-18"),np.datetime64("2024-01-19"),np.datetime64("2024-01-19")
,np.datetime64("2024-01-19"),np.datetime64("2024-01-20"),np.datetime64("2024-01-20"),np.datetime64("2024-01-20"),np.datetime64("2024-01-20")]

datesTail=[np.datetime64("2024-01-13"),np.datetime64("2024-01-14"),np.datetime64("2024-01-15")
,np.datetime64("2024-01-16"),np.datetime64("2024-01-17")
,np.datetime64("2024-01-18"),np.datetime64("2024-01-19"),np.datetime64("2024-01-20")
,np.datetime64("2024-01-21")]
#runs 138858 to 138882
...
ax = fig.add_subplot(gs[0])
tailLength = 25
# print("short run list",runList[-tailLength:])
ax.plot(runList[-tailLength:],HG7rateList[-tailLength:],marker="D",ls="-",lw = 2.5,ms=8,c=colorsCustom[1],label=r"IT7HG",alpha=1)
ax.plot(runList[-tailLength:],HG7NOHLCrateList[-tailLength:],marker="v",ls="-",lw = 2.5,ms=8,c=colorsCustom[2],label=r"IT7HG w/o ITSMT",alpha=1)
ax.plot(runList[-tailLength:],HG7WHLCrateList[-tailLength:],marker="^",ls="-",lw = 2.5,ms=8,c=colorsCustom[3],label=r"IT7HG w/ ITSMT",alpha=1)
ax.plot(runList[-tailLength:],HLCrateList[-tailLength:],marker="o",ls="-",lw = 2.5,ms=8,c=colorsCustom[4],label=r"ITSMT",alpha=1)

print("rate means",np.mean(HG7rateList[-tailLength:]),np.mean(HG7NOHLCrateList[-tailLength:]),np.mean(HG7WHLCrateList[-tailLength:]),np.mean(HLCrateList[-tailLength:]))
# rate means 24.783263990617883 19.063650427742253 5.719613562875635 6.517709119004611
ax.tick_params(axis='both',which='both', direction='in', labelsize=18)
secax = ax.secondary_xaxis('top')
secax.tick_params(axis='both',which='both', direction='in', labelsize=18)
ax.tick_params(axis='x', labelrotation=90)
secax.tick_params(axis='x', labelrotation=90)
ax.set_xticks(runList[-tailLength:])
ax.set_xticklabels(runList[-tailLength:])
secax.set_xticks(runList[-tailLength:][::3])
secax.set_xticklabels(datesTail)
# ax.set_xlabel(r"$Q_{tot}$ [m]", fontsize=22)
# ax.set_ylabel(r"$\theta$$^{\circ}$", fontsize=22)
ax.set_ylabel("rate [Hz]", fontsize=20)
# ax.set_xlabel("Q$_{tot} [VEM]$", fontsize=22)
ax.set_xlabel("run", fontsize=20)
secax.set_xlabel("day", fontsize=20)
# ax.set_yscale("log")
# ax.set_xlim(-600,600)
ax.set_ylim(0,35)
ax.legend(fontsize=16,ncol=2)
ax.grid(True,alpha=0.5)
plt.savefig(plotFolder+"/trigEventCountTail.pdf",transparent=False,bbox_inches='tight')
plt.savefig(plotFolder+"/trigEventCountTail.png",transparent=False,bbox_inches='tight')
