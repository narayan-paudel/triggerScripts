...
parser.add_argument('input', type=str, nargs='+', default="/data/user/enpaudel/triggerStudy/simFiles/dataSetClean1_6/FeDAT000001GenDetFiltProcUniqueCleanVEMEvts.i3.gz", help='Input files after running detector.py.')
args = parser.parse_args()

#usage python splitAndFilter7HG.py /data/user/enpaudel/triggerStudy/simFiles/dataSetClean1_6/FeDAT000001GenDetFiltProcUniqueCleanVEMEvts.i3.gz /data/user/enpaudel/triggerStudy/simFiles/dataSetClean1_6/FeDAT000001GenDetFiltProcUniqueCleanVEMEvts.i3.gz
fileName = args.input[0].split("/")[-1]
fileName = fileName.split(".")[0]+str(".i3.gz")
print(fileName)
...
fileList = sorted(glob.glob(fileDir+"*.i3.*"))[:1]
# fileName = "splitFilter.i3.gz"
...
class countFrames(icetray.I3Module):
  ...
  def DAQ(self,frame):
    self.qCount += 1
  def Finish(self):
    print("count,diff, ratio of Q and P",self.qCount,self.pCount,self.pCount-self.qCount,self.pCount/self.qCount)
...
tray.Add(Unify,"UnionHLCSLCTank",
  Keys=["OfflineIceTopHLCTankPulsesCleanTimeCleanCharge","OfflineIceTopSLCTankPulsesCleanTimeCleanCharge"],
  # Keys=["OfflineIceTopHLCVEMPulsesCleanTimeCleanCharge","OfflineIceTopSLCVEMPulsesCleanTimeCleanCharge"],
  Output='IceTopTankPulses',
  # Output='IceTopVEMPulses',
...
tray.AddModule(removePFrame,"rpf",
            # streams = [icetray.I3Frame.DAQ],
            )

tray.AddModule('I3TopClusterCleaning', name + '_' + "Inclined",
...
"MCPrimary","IceTopVEMPulses","IceTopTankPulses","HLC6_5000","H4aWeight",
"CleanIceTopRawData","HLC6","HG7_3"]
