...
# CORSIKA_FOLDER=/data/sim/IceTop/2009/generated/CORSIKA-ice-top/10410/7.0/
# CORSIKA_ID=DAT059871
# CORSIKA_FOLDER=/data/sim/IceTop/2009/generated/CORSIKA-ice-top/10410/5.0/
# CORSIKA_ID=DAT000001
# CORSIKA_FOLDER=/data/sim/IceTop/2009/generated/CORSIKA-ice-top/10410/5.5/
# CORSIKA_ID=DAT000006

# CORSIKA_ID=DAT000001
# CORSIKA_BZ=${CORSIKA_FOLDER}${CORSIKA_ID}.bz2

CORSIKA_ID_BZ=$(basename $1)
CORSIKA_ID=${CORSIKA_ID_BZ%.*}
# OUTPUT_FOLDER=/home/enpaudel/icecube/triggerStudy/simFiles/
OUTPUT_FOLDER=/home/enpaudel/icecube/triggerStudy/simFiles/timingDebug/
##########################################################################3
##########################################################################3
# bunzip2 -ckd $1 > $OUTPUT_FOLDER/temp/$CORSIKA_ID
if [ ! -f $OUTPUT_FOLDER/temp/$CORSIKA_ID ]; then
	bunzip2 -ckd ${CORSIKA_BZ} > $OUTPUT_FOLDER/temp/$CORSIKA_ID
fi

GCD=/home/acoleman/work/datasets/gcd-files/GCD-Survey-AntITScint_2020.02.24.i3.gz

discR=$(python -c "print(800+(int($ebin)-5)*300 + (2*(int($ebin)-5)//3)*300 + ((int($ebin)-5)//3)*300)")
GENERATOR_PY=$I3BUILD/simprod-scripts/resources/scripts/icetopshowergenerator.py
FLAGS="--UseGSLRNG --gcdfile ${GCD} --seed 0 --nproc 1 --samples 100 --r $discR"
FLAGS="$FLAGS --inputfile $OUTPUT_FOLDER/temp/$CORSIKA_ID"
FLAGS="$FLAGS --output $OUTPUT_FOLDER/ITGen/${CORSIKA_ID}Gen.i3.bz2"

echo running $GENERATOR_PY
echo flags $FLAGS
# $ICETRAY_ENV ${GENERATOR_PY} $FLAGS
python ${GENERATOR_PY} $FLAGS
...
FLAGS2+=" --inputfile $OUTPUT_FOLDER/ITGen/${CORSIKA_ID}Gen.i3.bz2"
FLAGS2+=" --output $OUTPUT_FOLDER/ITGen/${CORSIKA_ID}GenDet.i3.bz2"

echo running $DETECTOR_PY
echo flags $FLAGS2
python ${DETECTOR_PY} $FLAGS2
...
FLAGS3+=" --input $OUTPUT_FOLDER/ITGen/${CORSIKA_ID}GenDet.i3.bz2"
FLAGS3+=" --output $OUTPUT_FOLDER/ITGen/${CORSIKA_ID}GenDetFilt.i3.bz2"

echo running $FILTERING_PY
echo flags $FLAGS3

python ${FILTERING_PY} $FLAGS3
...
FLAGS4+=" --input $OUTPUT_FOLDER/ITGen/${CORSIKA_ID}GenDetFilt.i3.bz2"
FLAGS4+=" --output $OUTPUT_FOLDER/ITGen/${CORSIKA_ID}GenDetFiltProc.i3.bz2"

echo running $PROCESS_PY
echo flags $FLAGS4

python ${PROCESS_PY} $FLAGS4
